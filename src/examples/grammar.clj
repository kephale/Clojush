...
(defn -main [& args]
  (let [argmap (apply hash-map
	...
	error-fn (fn [program]
		   (list
		    (let [lang (deref (ns-resolve 'examples.grammar (:language argmap)))
			  pos ((grammar-eval (if (:with-empty-string argmap) (cons "" (first lang)) (first lang))
					     (repeat true))
			       program)
			  neg ((grammar-eval (second lang)
					     (repeat false))
			       program)]
